...
with open ("HuaweiWallet/train-wallet-v2-with-intent", 'rb') as fp:
...
for key in intent_query_dict:
    ...
    with open('walletdata_with_entities/intents/wallet.agent.' + key_slug + '.json', 'w') as outfile:
        json.dump(payload_template_response, outfile)

    with open('walletdata_with_entities/intents/wallet.agent.' + key_slug + '_usersays_en.json', 'w') as outfile:
...
for key in entity_value_list:
    ...
    with open('walletdata_with_entities/entities/' + key + '.json', 'w') as outfile:
    ...
    with open('walletdata_with_entities/entities/' + key + '_entries_en.json', 'w') as outfile:
