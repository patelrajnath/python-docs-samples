...
def detect_intent_texts(project_id, session_id, language_code):
    ...
    df = pd.read_csv('HuaweiAccount/200924_scenario2_test30_huawei_account.csv')
    ...
    pred_df = pd.Series(predictions, name='predicted_label')
    true_df = pd.Series(true_labels, name='true_label')
    input_df = pd.Series(input_text, name='text')

    results = pd.concat([input_df, true_df, pred_df], axis=1)
    results.to_csv('HuaweiAccount/results.csv')
